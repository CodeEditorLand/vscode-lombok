...
async function revealWorkspaceEdit(
...
export async function lombokAction(
			...
			return {
				...
				description:
					annotationsDescriptions[
						supportedLombokAnnotations.indexOf(name)
					],
				buttons: [
					{
					...
					disposables.push(
						pickBox.onDidTriggerItemButton((e) => {
							env.openExternal(
								Uri.parse(
									annotationLinks[
										supportedLombokAnnotations.indexOf(
											e.item.label.split("@")[1],
										)
									],
								),
							);
...
function getSelectedAnnotations(text: string): string[] {
	return supportedLombokAnnotations.filter((item) =>
		text.includes(`@${item}`),
	);
}
